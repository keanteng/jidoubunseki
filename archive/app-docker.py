...
class ModelServer:
    ...
    def setup_mlflow(self):
        ...
        # Force absolute path for Docker container
        import os
        
        # Check if we're in Docker (common environment variable)
        if os.path.exists('/app') or os.environ.get('DOCKER_CONTAINER'):
            # We're in Docker, use absolute Docker path
            tracking_uri = 'file:///app/mlruns'
            # Set environment variable to ensure MLflow uses Docker paths
        ...
        # Additional debugging for Docker
        if os.path.exists('/app'):
            logger.info("Running in Docker container")
            if os.path.exists('/app/mlruns'):
                logger.info(f"MLruns directory exists in Docker: {os.listdir('/app/mlruns')}")
            else:
                logger.warning("MLruns directory not found in Docker container")
        
        if self.config and 'mlflow' in self.config:
            experiment_name = self.config['mlflow'].get('experiment_name', 'loan_risk_prediction')
        else:
            experiment_name = 'loan_risk_prediction'
        ...
        try:
            ...
            if "deleted experiment" in str(e).lower():
                logger.warning(f"Experiment '{experiment_name}' was deleted. Creating new experiment with timestamp.")
                ...
                logger.info("Using Default experiment as fallback")
    ...
    def get_latest_production_model(self):
        ...
        try:
            ...
            for version in model_versions:
                # Fix: Handle tags properly - they might be dict or list of tag objects
                if hasattr(version, 'tags') and version.tags:
                    if isinstance(version.tags, dict):
                        tags = version.tags
                    else:
                        # Handle case where tags is a list of tag objects
                        tags = {tag.key: tag.value for tag in version.tags if hasattr(tag, 'key')}
                else:
                    tags = {}
    ...
    def load_model(self):
        ...
        try:
            ...
            # Override model URI for Docker if needed
            if os.path.exists('/app') or os.environ.get('DOCKER_CONTAINER'):
                ...
                if 'models:/' in model_uri or model_version:
                    ...
                    # Try different possible artifact paths in Docker
                    possible_paths = [
                        f"/app/mlruns/{run_id}/artifacts/model",
                        f"/app/mlruns/{run_id}/artifacts/local_model", 
                        f"/app/mlruns/{run_id}/artifacts/lightgbm-model",
                        f"/app/mlruns/{run_id}/artifacts",
                    ]
                    
                    # Add experiment-specific paths if we have experiment ID
                    if experiment_id:
                        possible_paths.extend([
                            f"/app/mlruns/{experiment_id}/{run_id}/artifacts/model",
                            f"/app/mlruns/{experiment_id}/{run_id}/artifacts/local_model",
                            f"/app/mlruns/{experiment_id}/{run_id}/artifacts/lightgbm-model",
                            f"/app/mlruns/{experiment_id}/{run_id}/artifacts",
                        ])
                ...
                for path in possible_paths:
                    if os.path.exists(path):
                        ...
                        if os.path.exists(os.path.join(path, "MLmodel")):
                            model_uri = f"file://{path}"
                            logger.info(f"Found MLmodel file at: {model_uri}")
                            break
                            ...
                            if os.path.basename(path) == 'local_model':
                                ...
                                if model_files:
                                    ...
                                    model_uri = f"local_model:{path}"
                                    logger.info(f"Using local_model path: {model_uri}")
                                    break
                            else:
                ...
                if model_uri is None:
                    ...
                    if os.path.exists(mlruns_dir):
                        logger.info(f"Available experiments in {mlruns_dir}: {os.listdir(mlruns_dir)}")
                        for exp_dir in os.listdir(mlruns_dir):
                            exp_path = os.path.join(mlruns_dir, exp_dir)
                            if os.path.isdir(exp_path):
                                logger.info(f"Runs in experiment {exp_dir}: {os.listdir(exp_path) if os.path.exists(exp_path) else 'None'}")
                    return False
            
            # Load the model
            if model_uri and model_uri.startswith('local_model:'):
                # Custom loading for local_model directory
                local_model_path = model_uri.replace('local_model:', '')
            ...
            try:
                ...
                # Debug: List available files in the artifacts directory
                if os.path.exists('/app') and model_version:
                    run_id = model_version.run_id
                    experiment_id = None
                    try:
                        client = mlflow.tracking.MlflowClient()
                    ...
                    debug_paths = [
                        f"/app/mlruns/{run_id}/artifacts/",
                        f"/app/mlruns/{experiment_id}/{run_id}/artifacts/" if experiment_id else None,
                    ]
                    
                    for debug_path in debug_paths:
                        if debug_path and os.path.exists(debug_path):
                            logger.info(f"Available files in {debug_path}: {os.listdir(debug_path)}")
                            # Check subdirectories too
                            for item in os.listdir(debug_path):
                                item_path = os.path.join(debug_path, item)
                                if os.path.isdir(item_path):
                                    logger.info(f"Files in {item_path}: {os.listdir(item_path)}")
            ...
            if model_version:
                if hasattr(model_version, 'tags') and model_version.tags:
                    if isinstance(model_version.tags, dict):
                        ...
                        # Handle case where tags is a list of tag objects
                        tags = {tag.key: tag.value for tag in model_version.tags if hasattr(tag, 'key')}
                else:
                    tags = {}
